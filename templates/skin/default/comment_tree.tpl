...
{if $oConfig->GetValue('plugin.opencomments.use_vk_api') or $oConfig->GetValue('plugin.opencomments.use_fb_api')}
	<script src="/plugins/opencomments/templates/skin/default/js/scripts.js" type="text/javascript"></script>
{/if}
{if $oConfig->GetValue('plugin.opencomments.use_vk_api')}
	<script src="http://vkontakte.ru/js/api/openapi.js" type="text/javascript"></script>
{/if}
{if $oConfig->GetValue('plugin.opencomments.use_fb_api')}
	<script src="http://connect.facebook.net/ru_RU/all.js" type="text/javascript"></script>
	<div id="fb-root"></div>
{/if}
{literal}
<script type="text/javascript">
	var vk_id = {/literal}{$oConfig->GetValue('plugin.opencomments.vk_id')}{literal};
	var fb_id = {/literal}{$oConfig->GetValue('plugin.opencomments.fb_id')}{literal};
</script>
<style>
	...
	.small_vk_icon {padding:2px 0 0 16px;background:url(/plugins/opencomments/templates/skin/default/images/auth_icons_small.png) -16px 0 no-repeat;}
	.small_fb_icon {padding:2px 0 0 16px;background:url(/plugins/opencomments/templates/skin/default/images/auth_icons_small.png) -64px 0 no-repeat;}
		...
		<div id="reply" class="reply">
			<form method="post" id="form_comment" onsubmit="return false;" enctype="multipart/form-data">	
				<div id="social_chooser">
					{$aLang.plugin.opencomments.opencomments_comment}: 
					<a class="small_vk_icon login" title="{$aLang.plugin.opencomments.opencomments_comment_vk}"></a>
					<a class="small_fb_icon login"  title="{$aLang.plugin.opencomments.opencomments_comment_fb}"></a>
				</div>
				<div id="social_info" style="display:none">
					{$aLang.plugin.opencomments.opencomments_hello}<span class="icon"></span> <span class="name"></span>
					(<a href="" id="sc_exit">{$aLang.plugin.opencomments.opencomments_exit}</a>)
				</div>
          		<div id="guest_input" style="padding-top:15px; padding-bottom:5px;"><b>{$aLang.plugin.opencomments.opencomments_name}:</b><input type="text" id="guest_name" class="input-text" name="guest_name" value="" style="width:200px;margin-left: 18px;" /></div>

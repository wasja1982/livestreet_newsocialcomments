...
<style>
	...
	.login.small_vk_icon, .login.small_fb_icon, .login.small_mr_icon, .login.vk_icon, .login.fb_icon, .login.mr_icon {cursor:pointer;margin: 0 3px 0 3px;   position: relative; top: 3px; }
	#social_info .name {padding-left:3px;}
    .guest-email,
    .guest-input { width: 49%; float: left; }
    .guest-input { margin-right: 1%; }
    .guest-comments-form {
        overflow: hidden;
        margin-bottom: 15px;
    }
    .guest-comments-captcha input {  width: 144px; margin-top: 5px; }
    .guest-comments-captcha img { margin-right: 10px; }

    @media only screen and (max-width: 479px) {
        .guest-input { margin-bottom: 10px; }
        .guest-email,
        .guest-input { width: 100% !important; }
    }
</style>
{/literal}
<script type="text/javascript">
...
<h4 class="reply-header" id="comment_id_0">
    <a href="#" class="{if $oConfig->GetValue('plugin.newsocialcomments.is_mobile')}button{else}button-3 full-width{/if}" onclick="ls.comments.toggleCommentForm(0);reloadCaptcha();return false;">{$sNoticeCommentAdd}</a>
</h4>
<div id="reply" class="reply">
    <form method="post" id="form_comment" onsubmit="return false;" enctype="multipart/form-data">
        {if $oConfig->GetValue('plugin.newsocialcomments.use_vk_api') or $oConfig->GetValue('plugin.newsocialcomments.use_fb_api') or $oConfig->GetValue('plugin.newsocialcomments.use_mr_api')}
            <div id="social_chooser" class="margin-10">
                <strong class="title-3 tm-1">{$aLang.plugin.newsocialcomments.newsocialcomments_comment}:</strong>
        ...
        <div class="guest-comments-form">
            <div id="guest_input" class="guest-input">
                <div class="input-icon">
                    <span class="font-icon-user btn"></span>
                    <input type="text" id="guest_name" name="guest_name" placeholder="{$aLang.plugin.newsocialcomments.newsocialcomments_name}" value="">
                </div>
            </div>
            {if $oConfig->GetValue('plugin.newsocialcomments.ask_mail')}
            <div id="guest_email" class="guest-email">
                <div class="input-icon">
                    <span class="font-icon-social-email btn"></span>
                    <input type="text" id="guest_email" name="guest_email" placeholder="{$aLang.plugin.newsocialcomments.newsocialcomments_mail}" value="">
                </div>
            </div>
            {/if}
        ...
        {hook run='form_add_comment_end'}
            ...
            <div id="captcha" class="guest-comments-captcha form-group">
                {hookb run="newsocialcomments_captcha"}
                <img src="{cfg name='path.root.engine_lib'}/external/kcaptcha/index.php?{$_sPhpSessionName}={$_sPhpSessionId}" id="commentCaptcha" onclick="this.src='{cfg name='path.root.engine_lib'}/external/kcaptcha/index.php?{$_sPhpSessionName}={$_sPhpSessionId}&n='+Math.random();" class="captcha-image align-left">
                    <input type="text" name="captcha" maxlength="3" placeholder="{$aLang.plugin.newsocialcomments.newsocialcomments_captcha}" value="">
                {/hookb}
            </div>

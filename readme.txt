Плагин "New Social Comments" (версия 1.0.6) для LiveStreet 1.0.3
...
16) $config['use_small_icon'] - Размер иконок для логина - 16px (true) или 24px (false).
По умолчанию 16px (true).

17) $config['use_parser'] - Использовать стандартный парсер для обработки комментариев (разрешает вставлять видео и т.п.).
Потенциально может снизить безопасность сайта. По умолчанию отключено (false).

18) $config['add_field'] - Добавлять в форму дополнительное поле для борьбы со спамом. По умолчанию разрешено (true).

19) $config['field_name'] - Название дополнительного поля для борьбы со спамом.
